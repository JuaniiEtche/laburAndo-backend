...
class resenaController {
  ...
  async vincular(req,res,next){
    ...
    const solicitudTrabajo={
      "idSolicitador": idPersona,
      "idPublicacion":idPublicacion
    }
    try{
    await SolicitudTrabajo.create(solicitudTrabajo);
  } catch(error){
    let e = new Error(`Hubo un error al crear la solicitud de trabajo`);
    e.statusCode = 500;
    throw e;
    }

    res.status(201).json({
      Mensaje: "Solicitud de tabajo creada con exito",
    });
  } catch (error) {
    next(error);
  }
